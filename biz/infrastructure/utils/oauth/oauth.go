...
func getTokenUrl(conf config.OauthConf, authType sts.AuthType, code string) string {
	...
	case sts.AuthType_gitee:
		return fmt.Sprintf("https://gitee.com/oauth/token?grant_type=authorization_code&code=%s&client_id=%s&redirect_uri=%s&client_secret=%s", code, conf.ClientId, "http://localhost:8888/auth/login/gitee", conf.Secret)
	default:
		return ""
	}
}
