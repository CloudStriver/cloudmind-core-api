...
func (s *StsService) ApplySignedUrl(ctx context.Context, req *core_api.ApplySignedUrlReq) (*core_api.ApplySignedUrlResp, error) {
	...
	genSignedUrlResp, err := s.PlatformSts.GenSignedUrl(ctx, &sts.GenSignedUrlReq{
		SecretId:  genCosStsResp.SecretId,
		SecretKey: genCosStsResp.SecretKey,
	...
	resp.Url = genSignedUrlResp.SignedUrl
	return resp, nil
}
